...
class SearchAndContents(BaseTool):
    ...
    description: str = (
        "Searches the web based on a search query for the latest results. Results are only from the last week. Uses the Exa API. This also returns the contents of the search results. Search only 2 articles at maximum. I should not exceed 2 articles."
    )
...
class FindSimilar(BaseTool):
    ...
    description: str = (
        "Searches for similar articles to a given article using the Exa API. Search only 2 articles at maximum. I should not exceed 2 articles. Takes in a URL of the article"
    )
